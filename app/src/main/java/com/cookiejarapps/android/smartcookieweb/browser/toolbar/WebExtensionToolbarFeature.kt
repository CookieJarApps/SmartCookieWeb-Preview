...
class WebExtensionToolbarFeature(
    ...
    private var extensionIds: List<String> = emptyList(),
    // Temporary flag for migration from old setting
    private var showAllExtensions: Boolean = false,
    ...
    override fun start() {
        ...
        webExtensionBrowserActions.filterKeys { !store.state.extensions.containsKey(it) || store.state.extensions[it]?.enabled == false }
        ...
        webExtensionPageActions.filterKeys { !store.state.extensions.containsKey(it) || store.state.extensions[it]?.enabled == false }
            .forEach { (extensionId, action) ->
    ...
    internal fun renderWebExtensionActions(state: BrowserState, tab: SessionState? = null) {
        ...
        extensions.filter { it.enabled }.filter {
            showAllExtensions || extensionIds.contains(it.id)
        }.sortedBy { it.name }.forEach { extension ->
